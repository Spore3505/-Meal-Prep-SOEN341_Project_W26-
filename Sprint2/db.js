...
async function initDb() {
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS recipes (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      owner_user_id INTEGER,          -- NULL means global recipe
      title TEXT NOT NULL,
      description TEXT,
  ...
  if ((countRow?.c || 0) === 0) {
    ...
    if (!seed) {
      // fallback seed (same idea you had in server.js before)
      seed = [
        {
    ...
    for (const r of seed) {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (st) await run(`INSERT INTO recipe_steps (recipe_id, step_index, step_text) VALUES (?,?,?)`, [recipeId, i, st]);
      }
    }
  }
}
