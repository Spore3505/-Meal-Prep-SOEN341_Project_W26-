...
const { initDb, run, get, all } = require("./src/models/db");

const app = express();

const view = (name) => path.join(__dirname, "src", "views", name);
...
// Serve public assets like /CSS, /js, /images
app.use(express.static(path.join(__dirname, "public")));

// Serve view-level assets if any are directly inside src/views
...
function buildHerbPool() {
  return ["parsley", "cilantro", "basil", "oregano", "mint", "chives"];
}
...
function buildSaucePool(spicy, dietary) {
  ...
  if (dietary === "dairy-free") {
    pool = pool.filter(
      (s) => !normalize(s).includes("yogurt") && !normalize(s).includes("creamy")
    );
  }
...
function buildRecipeFromAnswers({ answers, allergies, preferences, excludeTitles }) {
  ...
  if (!proteinPool.length) proteinPool = [{ name: "chickpeas", tags: ["vegan", "vegetarian", "dairy-free"] }];
  if (!basePool.length) basePool = ["rice"];
  if (vegPool.length < 3) vegPool = ["broccoli", "carrots", "spinach", "bell pepper"];
  if (!herbPool.length) herbPool = ["parsley"];
  if (!methodPool.length) methodPool = ["Bowl"];
  if (!saucePool.length) saucePool = ["lemon olive dressing"];
...
app.put("/recipes/:id", requireAuth, async (req, res) => {
  try {
    ...
    const existing = await get(`SELECT owner_user_id FROM recipes WHERE id = ?`, [id]);
